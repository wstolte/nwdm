...
http://al-276.xtr.deltares.nl:8080/geoserver/NWDM/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=NWDM:measurement_view&outputFormat=csv&maxFeatures=50



## Locations

All unique locations in the database. The table can be downloaded using the url: http://al-276.xtr.deltares.nl:8080/geoserver/NWDM/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=NWDM:location&outputFormat=csv
...
```{r}
locs <- readr::read_csv("http://al-276.xtr.deltares.nl:8080/geoserver/NWDM/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=NWDM:location&outputFormat=csv") %>% sf::st_as_sf(coords = c("x","y"), crs = 25831) %>%
...
All unique combinations of location and parameters. The table can be downloaded completeley using the url:  http://al-276.xtr.deltares.nl:8080/geoserver/NWDM/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=NWDM:p35location&outputFormat=csv

The following parameters are currently loaded in the database:

```{r}
parameters <- readr::read_csv("http://al-276.xtr.deltares.nl:8080/geoserver/NWDM/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=NWDM:p35location&outputFormat=csv") %>% distinct(p35code, p35preflabel)
...
```{r}
structure <- httr::parse_url("http://al-276.xtr.deltares.nl:8080/geoserver/NWDM/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=NWDM:location&outputFormat=csv")
...
```{r}
url = "http://al-276.xtr.deltares.nl:8080/geoserver/NWDM/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=NWDM%3Ameasurement_view&cql_filter=%28p35preflabel%3D%27Water%20body%20nitrate%27%20and%20location_code%3D%27DOOVBWT%27%29&outputFormat=csv"
...
http://al-276.xtr.deltares.nl:8080/geoserver/NWDM/ows?service=WFS&version=1.0.0&request=GetCapabilities

A description of the featuretypes (layers) can be requested by:
http://al-276.xtr.deltares.nl:8080/geoserver/NWDM/wfs?request=DescribeFeatureType
